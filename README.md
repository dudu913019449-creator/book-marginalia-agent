...
* Explicit save, daily synthesis, and daily-note persistence workflows.
* A dependency-free Python helper for safe vault initialization, append-only capture, and non-overwriting daily-note saves.
...
Synthesize the day's saved reading notes and display the result without saving it.

```text
保存今日札记。
```

Persist the reviewed daily note. An existing daily note is never overwritten.

---

## Daily note workflow

Generating and saving are intentionally separate actions:
...
“存一下”
↓
highlights/YYYY-MM-DD.md
↓
“生成今日札记”
↓
Agent 综合当天已保存的划线并展示结果
↓
“保存今日札记”
↓
daily/YYYY-MM-DD.md
...
“生成今日札记” only reads that day's saved highlights and displays a synthesis. It does not write a file. “保存今日札记” is the separate authorization to persist the reviewed result. An existing daily note is never overwritten.
...
### Save a reviewed daily synthesis

```bash
python3 skills/book-marginalia-agent/scripts/vault.py save-daily \
  --vault ~/Documents/BookMarginalia \
  --date 2026-08-28 \
  --input /path/to/daily-note.md
```

The command creates `daily/2026-08-28.md` and refuses to overwrite it if it already exists. The helper also refuses any vault path inside the skill directory or a Git repository.
...
札记才会保存到 `daily/YYYY-MM-DD.md`；如果当天的札记已经存在，它不会覆盖。
